...
public class LBSEventCompletedActivity extends BaseActivity implements OnClickListener {
  ...
  protected void onCreate(Bundle arg0) {
    ...
    if (intent != null) {
      ...
      if (Intents.ACTION_LBS_WAIT.equalsIgnoreCase(action)) {
        showWaiting(intent.getIntExtra(Intents.EXTRA_MY_POINT, 0), intent.getIntExtra(Intents.EXTRA_MY_TIME, 0));
  ...
  private void showWaiting(int point, int time) {
    ...
    mWaitSomebody.setText(String.format(getString(R.string.wait_somebody), mApplication.getCurrentPeer().getName()));
    mCurrentEventScoreValue.setText(String.format(getString(R.string.current_event_score_value), point, time));
  }
  
  private void showResult(boolean win, int myPoint, int myTime, int otherPoint, int otherTime, int winPoint, int winRate) {
    ...
    if (win) {
      ...
      mEventPoint.setText(String.format(getString(R.string.drop_point), winPoint));
    }
    
    mMyCurrentEventPoint.setText(String.format(getString(R.string.current_event_point), myPoint));

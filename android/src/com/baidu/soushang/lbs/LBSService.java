...
public class LBSService extends Service {
			...
			if (connectFuture.awaitUninterruptibly(1000)
				...
				mHandler.setClientListener(new ClientListener() {
					...
					public void onFightEnd(int result, int myPoint, int myTime,
							int myPointDelta, int myWinRate, int otherPoint,
							int otherTime) {
						...
						intent.putExtra(Intents.EXTRA_MY_POINT_DELTA,
								myPointDelta);
	...
	private void startHeartbeat() {
		...
		mHeartbeat = PendingIntent.getService(this, 0, heartbeatIntent,
				PendingIntent.FLAG_CANCEL_CURRENT);
		mAlarmManager.setRepeating(AlarmManager.RTC, new Date().getTime(),
				60000, mHeartbeat);
	}
